...
/*   Updated: 2016/10/13 12:46:25 by mdos-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_zi(void *arg, t_flag flag, int *r)
{
	...
	(flag.flag[' '] && !flag.flag['+'] && !flag.flag['-'] && ll > 0 && !flag.width && !flag.precision && ++*r) ? ft_putchar(' ') : 0;
	...
	else
	{
		...
		(negative == 1 && ++*r) ? ft_putchar('-') : 0;
		(flag.flag['-'] == 0 && flag.flag['0'] && !flag.precision) ? print_width_z(nb, r) : 0;
		(flag.p_given && !flag.precision && !ll) ? 0 : ft_putstr(arr + negative);
		*r += (flag.p_given && !flag.precision && !ll) ? 0 : ft_strlen(arr + negative);
	}
