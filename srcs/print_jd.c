...
/*   Updated: 2016/10/21 20:48:39 by mdos-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_jd(void *arg, t_flag flag, int *r)
{
	...
	if (flag.width > flag.precision)
	{
		...
		(*(intmax_t*)arg < 0 && flag.precision > (int)ft_strlen(arr))
			? --nb : 0;
	}
	(flag.flag[' '] && !flag.flag['+'] && !flag.flag['-'] && *(int*)arg > 0
		&& !flag.width && !flag.precision && ++*r) ? ft_putchar(' ') : 0;
	(flag.flag['+'] == 1 && *(intmax_t*)arg > 0) ? --nb : 0;
	(flag.flag['-'] == 0 && (!flag.flag['0'] || flag.precision))
		? print_width(nb, r) : 0;
	(flag.flag['+'] == 1 && *(intmax_t*)arg >= 0 && ++*r) ? ft_putchar('+') : 0;
