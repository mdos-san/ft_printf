...
/*   Updated: 2016/10/21 22:56:28 by mdos-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_jd(t_flag *flag, int *r)
{
	...
	if (flag->width > flag->precision)
	{
		...
		(j < 0 && flag->precision > (int)ft_strlen(arr))
			? --nb : 0;
	}
	(flag->flag[' '] && !flag->flag['+'] && !flag->flag['-'] && j > 0
		&& !flag->width && !flag->precision && ++*r) ? ft_putchar(' ') : 0;
	(flag->flag['+'] == 1 && j > 0) ? --nb : 0;
	(flag->flag['-'] == 0 && (!flag->flag['0'] || flag->precision))
		? print_width(nb, r) : 0;
	(flag->flag['+'] == 1 && j >= 0 && ++*r) ? ft_putchar('+') : 0;
